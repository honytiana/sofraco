...
const { getFileName } = require('../services/document/files');
...
exports.postMail = async (req, res) => {
    ...
    try {
        const excelMasters = await excelMasterHandler.getExcelMastersCourtierByYearMonth(req.body.courtier, req.body.year, req.body.month, 'zip');
        if (excelMasters.length > 0) {
            for (let excelMaster of excelMasters) {
                const attachementPath = excelMaster.path;
                const fileNameAttachement = getFileName(attachementPath);
